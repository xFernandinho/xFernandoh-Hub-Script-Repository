...
local frameWidth = isMobile and 0.85 or 0
local frameWidthOffset = isMobile and 0 or 320
local frameHeight = (isMobile and 460 or 500) + (isMobile and 32 or 34)
...
Title.Parent = Header
local closeSize = isMobile and 30 or 32
local closePos = isMobile and -38 or -40
...
CloseButton.TextSize = isMobile and 14 or 15
...
local rowCurrentJobIdHeight = isMobile and 26 or 28
local rowCurrentJobIdTop = headerHeight + 8
...
CurrentJobIdLabel.Text = "🆔 Job ID actual: " .. tostring(game.JobId)
CurrentJobIdLabel.TextColor3 = Color3.fromRGB(220, 255, 235)
CurrentJobIdLabel.TextSize = isMobile and 10 or 11
...
local copyCurrentJobIdSize = isMobile and 20 or 22
...
local row1Height = isMobile and 30 or 32
local row1Top = rowCurrentJobIdTop + rowCurrentJobIdHeight + 6
...
local rowFilterHeight = isMobile and 28 or 30
local rowFilterTop = row1Top + row1Height + 6
...
local rowJobIdHeight = isMobile and 28 or 30
local rowJobIdTop = rowFilterTop + rowFilterHeight + 6
...
local row2Height = isMobile and 24 or 26
local row2Top = rowJobIdTop + rowJobIdHeight + 6
...
local listTopOffset = row2Top + row2Height + 6
local bottomReserved = 36
...
local statusHeight = isMobile and 18 or 20
...
local function createServerEntry(serverData, index)
    local entryHeight = isMobile and 54 or 58
    local Entry = Instance.new("Frame")
    Entry.Name = "Entry_" .. index
    Entry.Size = UDim2.new(1, -8, 0, entryHeight)
    Entry.BackgroundColor3 = Color3.fromRGB(26, 30, 32)
    ...
    EntryStroke.Color = Color3.fromRGB(50, 140, 90)
    EntryStroke.Thickness = 1
    EntryStroke.Transparency = 0.8
    EntryStroke.Parent = Entry

    local playerCountWidth = isMobile and 44 or 48
    ...
    PlayerCount.TextSize = isMobile and 13 or 14
    PlayerCount.Font = Enum.Font.GothamBold
    PlayerCount.TextXAlignment = Enum.TextXAlignment.Left
    PlayerCount.Parent = Entry

    local joinButtonWidth = isMobile and 54 or 58
    local joinButtonHeight = isMobile and 28 or 30
    local copyButtonSize = isMobile and 20 or 22
    local rightReserved = joinButtonWidth + copyButtonSize + 22

    local ServerInfo = Instance.new("TextLabel")
    ServerInfo.Size = UDim2.new(1, -(playerCountWidth + 14 + rightReserved), 0.45, 0)
    ServerInfo.Position = UDim2.new(0, playerCountWidth + 14, 0, 4)
    ...
    ServerInfo.TextSize = isMobile and 9 or 10
    ...
    CopyButton.Position = UDim2.new(1, -(joinButtonWidth + copyButtonSize + 14), 0.5, -(copyButtonSize/2))
    CopyButton.BackgroundColor3 = Color3.fromRGB(30, 35, 33)
    CopyButton.Text = "📋"
    CopyButton.TextColor3 = Color3.fromRGB(150, 200, 170)
    CopyButton.TextSize = isMobile and 10 or 11
    ...
    JoinButton.Position = UDim2.new(1, -(joinButtonWidth + 8), 0.5, -(joinButtonHeight/2))
    ...
    JoinButton.TextSize = isMobile and 11 or 12
    ...
    local normalColor = isFull and Color3.fromRGB(110, 70, 30) or Color3.fromRGB(40, 140, 90)
    local hoverColor = isFull and Color3.fromRGB(140, 90, 40) or Color3.fromRGB(50, 170, 110)
...
local autoRefreshOptions = {"Off", "30s", "60s"}
...
local buttons = {RefreshButton, SortButton, AutoButton, ClearFilterButton, JoinJobIdButton, CopyCurrentJobIdButton, PrevButton, NextButton, CloseButton, ToggleButton}
